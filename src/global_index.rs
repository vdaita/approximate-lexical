...
impl GlobalIndex {
        ...
        let term_indices: Vec<TermIndex> = (0..max_term_index)
            ...
            .map(|term_index| {
                let term_documents: Vec<Vec<(usize, f32)>> = approximated_documents
                    ...
                    .collect();
                TermIndex::new(term_index, &term_documents, Arc::new(projector.clone()), parameters.clone())
            })
            .collect();
    ...
    fn query(&self, query: &Vec<(usize, f32)>, top_k_clusters: usize, top_k_elements: usize) -> Vec<(usize, f32)> {
        let projected_vector = self.projector.project(query);
        let query_weight_sum = query.iter().map(|&(_, value)| value).sum::<f32>();
        let normalized_query = query.iter().map(|&(term, value)| (term, value / query_weight_sum)).collect::<Vec<(usize, f32)>>();
        let top_k_segmented_clusters: Vec<&SegmentedCluster> = normalized_query
            ...
            .map(|&(term_index, weight)| {
                ...
                self.term_indices[term_index].get_segmented_clusters(&projected_vector, num_clusters)
            })
            .flatten_iter()
            .collect();
            ...
            .map(|cluster_segment| {
                let cluster_segments: Vec<&ClusterSegment> = top_k_segmented_clusters
                    ...
                    .map(|segmented_cluster| {
                        segmented_cluster.segments[cluster_segment]
                    })
                    .collect();
                
                let top_k_score_entries: Vec<ScoreHeapEntry> = GlobalIndex::weighted_top_k_lists(
                    query,
                    cluster_segments,
                    top_k_elements
                );
