...
use distances::vectors::{euclidean, squared_euclidean};
use rayon::prelude::*;
...
pub fn create_kmeans(
    ...
    if let Some(first) = data.choose(&mut rng) {
        ...
        while centroids.len() < k {
            let sampled_points = data.choose_multiple(&mut rng, kmeanspp_sample);
            ...
            let total_distance: f32 = sample_distances.sum();
